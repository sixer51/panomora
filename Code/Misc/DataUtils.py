...
def SetupAll(BasePath, CheckPointPath):
    ...
    LabelsPathTrain = './TxtFiles/LabelsTrainSupervised.txt'
...
def ReadLabels(LabelsPathTrain):
    if not (os.path.isfile(LabelsPathTrain)):
        ...
        TrainLabelReader = open(LabelsPathTrain, "r")
        # TrainLabels = TrainLabelReader.read()
        # TrainLabels = list(map(float, TrainLabels.split()))
        # print(TrainLabels)
        TrainLabels = []
        TrainLabel = TrainLabelReader.readline()
        # for _ in range(51):
        #     TrainLabel = TrainLabelReader.readline()
        #     # print(TrainLabel)
        #     TrainLabel = np.fromstring(TrainLabel, dtype=int, sep='.,')
        #     print(TrainLabel)
        while len(TrainLabel) > 0:
            # print(TrainLabel)
            TrainLabels.append(np.fromstring(TrainLabel, dtype=int, sep='.,'))
            TrainLabel = TrainLabelReader.readline()
